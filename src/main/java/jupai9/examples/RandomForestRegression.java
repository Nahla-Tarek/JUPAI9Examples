...
public class RandomForestRegression {
    public static void main(String[] args) {
        ...
        // Create Spark Session pto create connection to Spark
        ...
        vectorAssembler.setOutputCol("featuresss");
        ...
        airbnbDFTrainTransform.select("bedrooms", "featuresss", "price").show(10);

        RandomForestRegressor dt = new RandomForestRegressor();
        dt.setFeaturesCol("featuresss");
        dt.setLabelCol("price");

        Pipeline pipeline = new Pipeline()
                .setStages(new PipelineStage[]{vectorAssembler, dt});
        PipelineModel model = pipeline.fit(airbnbDFTrain.na().drop());
        final Dataset<Row> predictions = model.transform(airbnbDFTest.na().drop());
        predictions.show();
